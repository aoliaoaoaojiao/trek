...
var gameNavigationOCRExpectedControls = []fixtureExpectedControl{
	{name: "basic", aliases: []string{"basic"}, bounds: [4]float64{0.1950, 0.1646, 0.2900, 0.2593}},
	{name: "drag drop", aliases: []string{"drag drop"}, bounds: [4]float64{0.1950, 0.2904, 0.2893, 0.3851}},
	{name: "wait ui 2", aliases: []string{"wait ui 2"}, bounds: [4]float64{0.3291, 0.1646, 0.4235, 0.2593}},
	{name: "list view", aliases: []string{"list view"}, bounds: [4]float64{0.1950, 0.4115, 0.2893, 0.5062}},
	{name: "local", aliases: []string{"local"}, bounds: [4]float64{0.1950, 0.5419, 0.2893, 0.6366}},
	{name: "positioning", aliases: []string{"positioning"}, bounds: [4]float64{0.1950, 0.5419, 0.2893, 0.7578}},
	{name: "wait ui", aliases: []string{"wait ui"}, bounds: [4]float64{0.1950, 0.7488, 0.2893, 0.8736}},
	{name: "back", aliases: []string{"back"}, bounds: [4]float64{0.1614, 0.8736, 0.2857, 0.9984}},
}
...
func assertCandidatesHitExpectedControls(
	...
	for _, control := range expected {
		...
		t.Logf("命中控件 %s: text=%q bounds=[%.4f,%.4f,%.4f,%.4f] center=[%.4f,%.4f]",
			control.name, text, item.Command.Pos.Left, item.Command.Pos.Top, item.Command.Pos.Right, item.Command.Pos.Bottom, centerX, centerY)
	}
}
...
func extractOCRCandidateText(item perception.Candidate) string {
