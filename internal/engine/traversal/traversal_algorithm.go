...
type TraversalAlgorithm interface {
	...
	//
	// 返回的候选 Source 字段应设为 candidate.SourceAlgorithm，
	// Intent 字段应描述候选的语义意图（如"点击搜索按钮"），
	// Confidence 应基于算法内部的评估给出 0-1 分数。
	ProposeCandidates(ctx enginestate.TraversalContext) ([]candidate.Candidate, error)
	...
	// Reuse Q 值等）从中选择最优动作。
	SelectAction(ctx enginestate.TraversalContext, candidates []candidate.Candidate) (*types.ActionCommand, error)
